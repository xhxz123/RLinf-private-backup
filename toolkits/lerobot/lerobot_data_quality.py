...
``exclude_a2d.txt`` file; warnings requiring human judgement are written to
``review_a2d.txt``.

Example::

    uv run toolkits/lerobot/lerobot_data_quality.py \
...
Use the generated exclusion file for both normalization and training::
    ...
    uv run scripts/agibot_g01_multi_train.py compute-norm ... \
      --exclude-file reports/g01_preflight/exclude_a2d.txt
...
_EPISODE_RE = re.compile(r"episode[_-](\d+)\.parquet$")


def _episode_index_from_path(path: str | pathlib.Path) -> int:
    match = _EPISODE_RE.search(pathlib.Path(path).name)
    if match is None:
        raise ValueError(f"Could not parse episode index from {path!s}")
...
def _video_jobs(spec: DatasetSpec, info: dict[str, Any], lengths: dict[int, int]) -> list[VideoJob]:
    ...
    for episode_index, expected_frames in sorted(lengths.items()):
        ...
        for video_key in video_keys:
            chunk_path = spec.root / "videos" / f"chunk-{episode_chunk:03d}" / video_key / f"episode_{episode_index:06d}.mp4"
            flat_path = spec.root / "videos" / video_key / f"episode_{episode_index:06d}.mp4"
            path = chunk_path if chunk_path.exists() else flat_path
...
def _write_list(path: pathlib.Path, results: Sequence[EpisodeResult], *, review: bool) -> None:
    ...
    lines = [
        f"# AgiBot G01 {label} list generated by toolkits/lerobot/lerobot_data_quality.py",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--exclude-review", action="store_true", help="Also put review episodes in exclude_a2d.txt.")
...
def main() -> None:
    ...
    for spec in specs:
        ...
        parquet_files = sorted((spec.root / "data").rglob("*.parquet"))
        if not parquet_files:
            raise FileNotFoundError(f"No parquet files found under {spec.root / 'data'}")

        declared_ids = set(lengths)
        actual_ids = {_episode_index_from_path(path) for path in parquet_files}
    ...
    _write_list(output_dir / "exclude_a2d.txt", exclusion_output, review=False)
    _write_list(output_dir / "review_a2d.txt", reviews, review=True)
    ...
    print(f"Use exclusion file: {output_dir / 'exclude_a2d.txt'}")
